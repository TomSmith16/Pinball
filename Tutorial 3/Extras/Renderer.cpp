...
namespace VisualDebugger
{
	namespace Renderer
	{
		...
		void Render(PxActor** actors, const PxU32 numActors)
		{
			...
			//Use if statement to check if the visualization is true or false, allows for transparent triggers
			...
			for(PxU32 i=0;i<numActors;i++)
			{
				...
				else if (actors[i]->isRigidActor())
				{
					...
					for(PxU32 j = 0; j < shapes.size(); j++)
					{
						...
						PxShapeFlags flags = shapes[j]->getFlags();
						//Check for triggers to make them transparent
						if ((flags & PxShapeFlag::eVISUALIZATION) == false)
							continue;
